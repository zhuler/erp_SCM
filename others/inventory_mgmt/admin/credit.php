...
<tbody>
    <?php $get_type = mysql_query("SELECT DISTINCT receipt FROM sales WHERE type = 'Credit'") or die(mysql_error());
	$sn = 1;
	while($rst = mysql_fetch_array($get_type)){
	$receipt = $rst['receipt'];
	$getrec = mysql_query ("SELECT * FROM sales_rec WHERE receipt = '$receipt'") or die(mysql_error());
	$sumtotal = 0;
	$sumpaid = 0;
	$sumbal = 0;
	$row = mysql_fetch_array($getrec);
	$ID = $row['id'];
	$receipt  = $row['receipt'];
